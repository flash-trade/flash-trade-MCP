...
pub async fn execute(
    ...
    // collateral_delta_usd is in native USD (6 decimals)
    let collateral_delta_usd = usd_to_native(amount);

    let prog_id = program_id(&settings.cluster);
    let client = PerpetualsClient::new(prog_id, false);
    let ix_result = client.remove_collateral(
        &owner,
        &target_sym,
        &collateral_sym,
    ...
    print_preview(&[
        (
            "Operation",
            format!("Remove Collateral from {target_sym}/{collateral_sym}"),
        ),
        ("Position", position_str.to_string()),
        ("Amount", colors::format_usd(amount)),
    ]);
    ...
    let sig = TxEngine::execute(
        ...
        &keypair,
        &alt_pubkeys,
        compute_units::REMOVE_COLLATERAL,
    ...
    println!(
        "{}",
        formatter.success(&format!("Collateral removed!\n  Signature: {sig}"))
    );
    Ok(())
}
